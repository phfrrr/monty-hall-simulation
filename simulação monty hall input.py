...
# Simulação
mudanca = input(f"Deseja continuar com a quantia de {total_simulacoes}? (s/n): ").strip().lower()
if mudanca == 'n':
    total_simulacoes = int(input('Quantas vezes? '))
...
resposta = input("Deseja ver o histórico? (s/n): ").strip().lower()
if resposta == "s":
    print("Histórico geral:")
    for i in range(len(historico_porta_do_premio)):
     #histórico grupo 1
        print(f'\n{i+1}ª experiência:')
        print(f'\nNa {i+1}ª experiência, a porta sorteada foi {historico_porta_do_premio[i]}')
        print(f'\nNa {i+1}ª experiência, a escolha do grupo 1 foi {historico_escolhas_grupo_1[i]}')
        if historico_escolhas_grupo_1 [i] == historico_porta_do_premio [i]:
            print(f'\nGrupo 1 acertou na {i+1}ª experiência')
        else:
            print(f'\nGrupo 1 errou na {i+1}ª experiência')
        print(f'\nNa {i+1}ª experiência, a primeira escolha foi {historico_primeiras_escolhas_grupo_2[i]}')
        print(f'\nNa {i+1}ª experiência, a segunda escolha foi {historico_segundas_escolhas_grupo_2[i]}')
        if historico_escolhas_grupo_1 [i] == historico_porta_do_premio [i]:
            print(f'\nGrupo 1 acertou na {i+1}ª experiência')
        else:
            print(f'\nGrupo 1 errou na {i+1}ª experiência')
